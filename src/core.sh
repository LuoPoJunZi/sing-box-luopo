# ==========================================
# Sing-box-LuoPo Core Business Logic
# ==========================================
...
is_main_menu() {
    ...
    while :; do
        clear
        echo -e "\e[96m=====================================================\e[0m"
        echo -e "\e[96m          Sing-box-LuoPo 魔改管理面板 $is_sh_ver\e[0m"
        echo -e "\e[96m=====================================================\e[0m"
...
main() {
    ...
    dd | ddel | fix | fix-all)
        ...
        fix)
            ...
            ;;
        fix-all)
            is_dont_auto_exit=1; msg
            for v in $(ls $is_conf_dir | grep .json$ | sed '/dynamic-port-.*-link/d'); do
                msg "fix: $v"; change $v full
            done
            _green "\nfix 完成.\n"
            ;;
        *)
    ...
    u | up | update | U | update.sh)
        is_update_name=$2; is_update_ver=$3
        if [[ ! $is_update_name ]]; then is_update_name=core; fi
        if [[ $1 == 'U' || $1 == 'update.sh' ]]; then is_update_name=sh; is_update_ver=; fi
    ...
    t | test) get test-run ;;
    reinstall) get $1 ;;
    get-port) get_port; msg $tmp_port ;;
    main) is_main_menu ;;
    v | ver | version)
        ...
        ;;
    h | help | --help) load help.sh; show_help ${@:2} ;;
    *)
        is_try_change=1
        change test $1
        if [[ $is_change_id ]]; then
